    ...
    <el-dialog @close="clearForm" :title="title" :visible.sync="dialogFormVisible" :close-on-click-modal="false">
      <el-form :model="roleForm" ref="roleFormRef" :rules="rules">
        ...
        <el-form-item prop="menuIdList" label="权限设置" :label-width="formLabelWidth">
          <el-tree :data="menuList" :props="menuProps" node-key="menuId" show-checkbox style="width:85%"
            default-expand-all  ref="menuRef"></el-tree>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    saveRole() {
      ...
      this.$refs.roleFormRef.validate((valid) => {
        if (valid) {
          // 提交保存请求
          let checkedkey=this.$refs.menuRef.getCheckedKeys();
          let halfcheckedkey=this.$refs.menuRef.getHalfCheckedKeys();
          this.roleForm.menuIdList=checkedkey.concat(halfcheckedkey);
